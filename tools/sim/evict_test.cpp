...
static std::vector<int> trumpetLoadOrder() {
  ...
  for (int ni = 0; ni < 12; ni++)
    for (int oct = 3; oct <= 5; oct++) {
      ...
      if (m >= 52 && m <= 83) midis.push_back(m);
    }
  return midis;
}
...
int main() {
  ...
  }
  {
    ...
    gB.clear();
    gB.add(mk(midiToHz(48)));
    for (int i = 0; i < 14; i++) gB.add(mk(midiToHz(72 + i)));
    gB.add(mk(midiToHz(96)));
    char msg[64]; snprintf(msg, sizeof(msg), "(共 %d 組)", gB.n);
    check("填滿 16 組", gB.n == 16, msg);
    const int v = gB.evictionTarget(midiToHz(60));
  ...
  {
    gB.clear();
    for (int i = 0; i < 12; i++) gB.add(mk(midiToHz(60 + i)));
