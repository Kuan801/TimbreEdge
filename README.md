...
TimbreEdge is a self-contained embedded instrument built on a Teensy 4.1 microcontroller with a Teensy Audio Shield (SGTL5000 codec). It handles recording, signal analysis, and 8-voice real-time synthesis entirely on-device to turn any audio input into a playable instrument, played via a USB computer keyboard or demonstrated through an integrated three-voice canon auto-playback mode.
...
Teensy 4.1 with a Teensy Audio Shield Rev D, a microSD card, an electret microphone on the shield's MIC pads, and a 128x64 I2C OLED. Optionally a USB A socket wired to the USB Host header.
...
TimbreEdge is one half of a two-part capstone project built by a four-person team. Chia-Hui Yang and I-Pei Chen were primarily responsible for TimbreEdge (this repository); Xuan-Yun Weng and Wen-Ting Chen were primarily responsible for the PC-based [Timbre Shadowing ](https://github.com/JamesWengCode/Intelligent_Electronic_Instruments_for_Timbre_Learning_and_Audio_Synthesis_TimbreShadowing) system, which classifies the instrument or instrument family from input audio and uses the identified timbral characteristics to support synthesis and playable keyboard output. All four members contributed to system integration, testing, and the project report.
